...
pub async fn import_from_gnote(db: &Database, gnote_dir: &str) -> Result<Vec<Note>> {
    ...
    for gnote_note in gnote_notes {
        ...
        let saved_note = db.create(note.clone()).await?;
        #[cfg(not(feature = "lite"))]
        if let Err(e) = db.upsert_note_to_vector(&saved_note).await {
            error!("Failed to upsert note to vector: {}", e);
        }
...
pub async fn export_to_gnote(db: &Database, note_id: &str, output_dir: &str) -> Result<String> {
    // Gnote uses same format as Tomboy
    export_to_tomboy(db, note_id, output_dir).await
}

pub async fn batch_import_gnote(db: &Database, files: Vec<String>) -> Result<Vec<Note>> {
    // Gnote uses same format as Tomboy
    batch_import_tomboy(db, files).await
}

async fn batch_import_tomboy(db: &Database, files: Vec<String>) -> Result<Vec<Note>> {
    let mut imported_notes = Vec::new();

    for file_path in files {
        if let Ok(note) = TomboyNote::parse_xml(Path::new(&file_path)) {
            let memo_note = note.to_memo_rs_note();
            let saved = db.create(memo_note.clone()).await?;
            #[cfg(not(feature = "lite"))]
            if let Err(e) = db.upsert_note_to_vector(&saved).await {
                error!("Failed to upsert note to vector: {}", e);
            }
...
pub async fn export_to_tomboy(db: &Database, note_id: &str, output_dir: &str) -> Result<String> {
