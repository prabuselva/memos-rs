...
use crate::models::{AuthError, Notebook, NotebookNode, Note, UserProfile};
...
pub fn create_router() -> Router<Arc<AppState>> {
    ...
    Router::new()
        ...
        .route("/notebooks/reorder", put(reorder_notebooks))
...
#[cfg(all(feature = "lite", not(feature = "llm")))]
async fn update_profile(
    CurrentUser(current_user): CurrentUser,
    State(state): State<Arc<AppState>>,
    Json(payload): Json<UpdateProfileRequest>,
...
async fn update_llm_settings(
    ...
    if let Some(api_key) = payload.get("api_key") {
