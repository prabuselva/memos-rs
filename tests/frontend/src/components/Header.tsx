...
import { notesApi, importExportApi, api, vectorDbApi, type VectorDBStatus } from '../lib/api';
import { ImportProgressModal } from './ImportProgressModal';
import { UserMenu } from './UserMenu';
import { SearchResults } from './SearchResults';
import { VectorDBStatusModal } from './VectorDBStatusModal';
...
export const Header = ({ onNewNote, onMenuToggle, onOpenProfile, onOpenLLMSettings, onOpenChat }: HeaderProps) => {
  const { setError, setNotes, searchQuery, setSearchQuery, searchResults, setSearchResults, setIsSearching, isSearching, selectNote, searchMode, setSearchMode } = useNoteStore();
  ...
  const [vectorDBStatus, setVectorDBStatus] = useState<VectorDBStatus | null>({ enabled: false, available: false, message: 'Checking...' });
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (isSearchOpen) {
        const isInsideDropdown = searchDropdownRef.current?.contains(event.target as Node);
        const isInsideSearchInput = searchInputRef.current?.contains(event.target as Node);
  ...
  const performSearch = useCallback(async () => {
    ...
    try {
      const response = searchMode === 'vector'
        ? await notesApi.vectorSearch(query)
        : await notesApi.search(query);
      setSearchResults(response.data);
      setSelectedIndex(0);
    } catch (error) {
      console.error('Search failed:', error);
      if (searchMode === 'vector' && !vectorDBStatus?.available) {
        setError('Vector search unavailable. Switch to SQLite search or enable Vector DB.');
      }
      setSearchResults([]);
    } finally {
      setIsSearching(false);
    }
  }, [searchQuery, searchMode, vectorDBStatus, setSearchResults, setIsSearching, setError]);
      ...
      <div className="relative flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex-shrink-0">
        ...
        <div className="flex-1 px-3 md:px-6 flex justify-center">
          <div className="relative w-full max-w-md" ref={searchInputRef}>
           ...
           {isSearchOpen && (
                <div ref={searchDropdownRef} className="absolute top-full mt-2 w-full rounded-lg bg-white dark:bg-gray-800 shadow-xl ring-1 ring-black ring-opacity-5 dark:ring-white dark:ring-opacity-10 z-[60] overflow-hidden flex flex-col max-h-[80vh]">
                  <div className="px-4 py-2 border-b border-gray-100 dark:border-gray-700 flex-shrink-0">
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-1 sm:gap-2 p-1 bg-gray-100 dark:bg-gray-700 rounded-lg" ref={searchModeRef}>
                        <button
                           type="button"
                           onMouseDown={(e) => {
                             e.preventDefault();
                           ...
                           disabled={!vectorDBStatus?.enabled}
                           className={`flex-1 px-3 py-1.5 text-sm font-medium rounded-md transition-all ${
                             searchMode === 'sql'
                               ? 'bg-white dark:bg-gray-600 text-gray-900 dark:text-gray-100 shadow-sm'
                               : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'
                           } ${!vectorDBStatus?.enabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                         >
                           SQL
                         </button>
                         <button
                           ...
                           className={`flex-1 px-3 py-1.5 text-sm font-medium rounded-md transition-all ${
                             searchMode === 'vector'
                               ? 'bg-white dark:bg-gray-600 text-gray-900 dark:text-gray-100 shadow-sm'
                               : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'
                           } ${!vectorDBStatus?.available ? 'opacity-50 cursor-not-allowed' : ''}`}
                         >
                           Vector
                         </button>
                      </div>
        ...
        {vectorDBStatus?.enabled && (
           <div className="flex items-center gap-2 md:gap-3 flex-shrink-0 px-3">
             <button
               ...
               title={!vectorDBStatus?.available ? 'AI Features disabled - Vector DB not available' : 'Open Chat (Ctrl+K)'}
               disabled={!!(!vectorDBStatus?.available)}
             >
         ...
         <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
           ...
           <UserMenu onOpenProfile={onOpenProfile} onOpenLLMSettings={onOpenLLMSettings} onOpenChat={handleOpenChat} vectorDBStatus={vectorDBStatus ?? undefined} />
         </div>
      </div>
      <ImportProgressModal
