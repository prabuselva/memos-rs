...
import type { SearchMode } from '../hooks/useNoteStore';
import { authApi, vectorDbApi, type VectorDBStatus } from '../lib/api';
...
export const Profile = ({ onClose }: ProfileProps) => {
  ...
  const { setNotes, searchMode, setSearchMode } = useNoteStore();
 ...
 useEffect(() => {
  ...
  const handleUpdateSearchMode = async (mode: SearchMode) => {
    try {
      await authApi.updateSearchMode(mode);
      setSearchMode(mode);
      setError(null);
    } catch (error) {
      console.error('Failed to update search mode:', error);
      setError('Failed to save search mode preference');
    }
  };
          ...
          <div className="space-y-4">
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
              <button
                ...
                className="text-left text-sm font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400"
              >
                {showSearchSettings ? 'Hide' : 'Search Settings'}
              </button>
              ...
              {showSearchSettings && (
                <div className="mt-4 space-y-4 animate-in fade-in slide-in-from-top-2">
                  <div>
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-1 sm:gap-2 p-1 bg-gray-100 dark:bg-gray-700 rounded-lg">
                       <button
                         type="button"
                         onClick={() => handleUpdateSearchMode('sql')}
                       ...
                       <button
                         type="button"
                         onClick={() => handleUpdateSearchMode('vector')}
