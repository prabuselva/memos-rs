...
use crate::models::{AuthResult, AuthError, User, Session, PasswordRecovery};
...
impl AuthRepository {
    ...
    pub async fn register(
        ...
        db.create_user(user.clone()).await.map_err(|e| {
    ...
    pub async fn verify_credentials(
        ...
        let user = db.get_user_by_username(username).await.map_err(|_| {
    ...
    pub async fn create_session(
        ...
        db.create_session(session.clone()).await.map_err(|e| {
            AuthError::Database(format!("Failed to create session: {}", e))
        })?;
    ...
    pub async fn get_session(
        ...
        let session = db.get_session(session_token).await.map_err(|_| {
    ...
    pub async fn invalidate_session(
        ...
        db.invalidate_session(session_token).await.map_err(|e| {
            AuthError::Database(format!("Failed to invalidate session: {}", e))
        })
    }
    ...
    pub async fn create_password_recovery(
        ...
        db.create_password_recovery(recovery.clone()).await.map_err(|e| {
    ...
    pub async fn get_password_recovery(
        ...
        let recovery = db.get_password_recovery(token).await.map_err(|_| {
            AuthError::SessionNotFound
        })?;
    ...
    pub async fn mark_password_recovery_as_used(
        ...
        db.mark_password_recovery_as_used(token).await.map_err(|e| {
            AuthError::Database(format!("Failed to mark recovery as used: {}", e))
        })
    }
    ...
    pub async fn get_user_by_id(
        ...
        db.get_user_by_id(user_id).await.map_err(|e| {
            AuthError::Database(format!("Failed to get user: {}", e))
        })
    }
    ...
    pub async fn update_user_password(
        ...
        let user = db.get_user_by_id(user_id).await.map_err(|e| {
            AuthError::Database(format!("Failed to get user: {}", e))
        })?;
        ...
        db.update_user(updated_user).await.map_err(|e| {
            AuthError::Database(format!("Failed to update password: {}", e))
        })?;
